...
public class SSInvoiceMath extends SSSaleMath{
    ...
    public static int getNumDelayedDays(SSInvoice iInvoice){
        ...
        if(iLastPayment == null || iPaymentDay == null ) return 0;
                ...
                if (iParcelProducts.contains(iRow.getProductNr())) {
                    ...
                    if (iProduct != null) {
                        for (SSProductRow iProductRow : iProduct.getParcelRows()) {
                            iReserved = iInvoiceCount.get(iProductRow.getProductNr()) == null ? iProductRow.getQuantity()*iRow.getQuantity() : iInvoiceCount.get(iProductRow.getProductNr()) + (iProductRow.getQuantity()*iRow.getQuantity());
                            iInvoiceCount.put(iProductRow.getProductNr(), iReserved);
                        }
                    }
                } else {
                    iReserved = iInvoiceCount.get(iRow.getProductNr()) == null ? iRow.getQuantity() : iInvoiceCount.get(iRow.getProductNr()) + iRow.getQuantity();
