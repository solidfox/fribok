...
public class SSSupplierInvoiceMath {
    ...
    public static BigDecimal getNetSum(SSSupplierInvoice iSupplierInvoice){
        ...
        for (SSSupplierInvoiceRow iRow : iSupplierInvoice.getRows()) {
            ...
            if(iRowSum != null) iTotalSum = iTotalSum.add( iRowSum );
    ...
    public static void calculateSaldos(){
        if(iSaldoMap == null) iSaldoMap = new HashMap<Integer,BigDecimal>();
    ...
    public static List<SSSupplierInvoice> getInvoicesForSupplier(SSSupplier iSupplier, Date iDate) {
        return getInvoicesForSupplier(SSDB.getInstance().getSupplierInvoices(), iSupplier, iDate);
    }
                ...
                if (iParcelProducts.contains(iRow.getProductNr())) {
                    ...
                    if (iProduct != null) {
                        for (SSProductRow iProductRow : iProduct.getParcelRows()) {
                            iReserved = iSupplierInvoiceCount.get(iProductRow.getProductNr()) == null ? iProductRow.getQuantity()*iRow.getQuantity() : iSupplierInvoiceCount.get(iProductRow.getProductNr()) + (iProductRow.getQuantity()*iRow.getQuantity());
                            iSupplierInvoiceCount.put(iProductRow.getProductNr(), iReserved);
                        }
                    }
                } else {
                    iReserved = iSupplierInvoiceCount.get(iRow.getProductNr()) == null ? iRow.getQuantity() : iSupplierInvoiceCount.get(iRow.getProductNr()) + iRow.getQuantity();
